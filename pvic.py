...
# question:
# use IBOTHead with smaller dimension or ?
# use verb_embedding as heads??
...
class FeatureHead(nn.Module):
    def __init__(self, dim, dim_backbone, return_layer, num_layers, net_type):
        super().__init__()
        self.dim = dim
        ...
        self.masked_embed = nn.Parameter(torch.zeros(dim))
    
    def generate_mask(self, mask, ratio=0.75):
        # ratio is keeped area ratio
        false_part = mask[~mask]
        mask_weight = torch.ones(len(false_part))
        selected_idx = torch.multinomial(mask_weight, int(len(false_part) * ratio))
        false_part[selected_idx] = True
        new_mask = mask.clone()
        new_mask[~mask] = false_part
        return new_mask
    def forward(self, x):
        pyramid = OrderedDict(
            (f"{i}", x[i].tensors)
        ...
        if self.net_type == 's' and self.training:
            x1 = x.clone()
            new_mask = self.generate_mask(mask=mask, ratio=0.75)
            new_mask = ~new_mask
            x1.permute(0, 2, 3, 1)[new_mask, :] = self.masked_embed.to(x.dtype)
            x1 = self.layers(x1)

            x2 = x.clone()
            new_mask = self.generate_mask(mask=mask, ratio=0.75)
            new_mask = ~new_mask
            x2.permute(0, 2, 3, 1)[new_mask, :] = self.masked_embed.to(x.dtype)
            x2 = self.layers(x2)
            return [x1, x2], mask
        else:
            x = self.layers(x)
...
class PViC(nn.Module):
    ...
    def __init__(self,
        detector: str,
        feature_head: FeatureHead, ho_matcher: nn.Module,
        ...
        self.patch_head = nn.Sequential(
            nn.Linear(self.feature_head.dim, self.feature_head.dim * 4), 
            nn.ReLU(),
            nn.Linear(self.feature_head.dim * 4, self.feature_head.dim * 4))
    ...
    def forward(self,
        images: List[Tensor], results:List[dict], detr_final_hs:Tensor, backbone_features:Tensor, k_pos:Tensor,
        targets: Optional[List[dict]] = None,
        llava_answer: Optional[List[Tensor]] = None,
        llava_feature:Optional[List[Tensor]] = None,
        ...
        memory, mask = self.feature_head(backbone_features)
        if self.net_type == 't':
            b, h, w, c = memory.shape
            memory = memory.reshape(b, h * w, c)
            kv_p_m = mask.reshape(-1, 1, h * w)
        elif self.net_type == 's' and self.training:
            b, h, w, c = memory[0].shape
            memory[0] = memory[0].reshape(b, h * w, c)
            memory[1] = memory[1].reshape(b, h * w, c)
            kv_p_m = mask.reshape(-1, 1, h * w)
        elif self.net_type == 's' and (not self.training):
            b, h, w, c = memory.shape
            memory = memory.reshape(b, h * w, c)
            kv_p_m = mask.reshape(-1, 1, h * w)
        region_props = prepare_region_proposals(
            results, detr_final_hs, image_sizes,
        ...
        if self.net_type == 't':
            ...
            memory = self.patch_head(memory)
            return query_embeds, logits[-1], memory
        elif self.net_type == 's' and self.training:
            # Enhance visual context with triplet decoder.
            query_embeds1 = []
            for i, (ho_q, mem) in enumerate(zip(ho_queries, memory[0])):
                query_embeds1.append(self.decoder(
                    ho_q.unsqueeze(1),              # (n, 1, q_dim)
                    mem.unsqueeze(1),               # (hw, 1, kv_dim)
                    kv_padding_mask=kv_p_m[i],      # (1, hw)
                    q_pos=positional_embeds[i],     # centre: (n, 1, 2*kv_dim), box: (n, 1, 4*kv_dim)
            ...
            # Concatenate queries from all images in the same batch.
            query_embeds1 = torch.cat(query_embeds1, dim=1)   # (ndec, \sigma{n}, q_dim)
            logits1 = self.binary_classifier(query_embeds1)

            # Enhance visual context with triplet decoder.
            query_embeds2 = []
            for i, (ho_q, mem) in enumerate(zip(ho_queries, memory[1])):
                query_embeds2.append(self.decoder(
                    ho_q.unsqueeze(1),              # (n, 1, q_dim)
                    mem.unsqueeze(1),               # (hw, 1, kv_dim)
                    kv_padding_mask=kv_p_m[i],      # (1, hw)
                    q_pos=positional_embeds[i],     # centre: (n, 1, 2*kv_dim), box: (n, 1, 4*kv_dim)
            ...
            # Concatenate queries from all images in the same batch.
            query_embeds2 = torch.cat(query_embeds2, dim=1)   # (ndec, \sigma{n}, q_dim)
            logits2 = self.binary_classifier(query_embeds2)
        elif self.net_type == 's' and (not self.training):
            # Enhance visual context with triplet decoder.
            query_embeds = []
            for i, (ho_q, mem) in enumerate(zip(ho_queries, memory)):
            ...
            query_embeds = torch.cat(query_embeds, dim=1)   # (ndec, \sigma{n}, q_dim)
            logits = self.binary_classifier(query_embeds)
        if self.training and self.net_type=='s':
            labels = associate_with_ground_truth(
                boxes, paired_inds, targets, self.num_verbs
            )
            cls_loss1, n_p = self.compute_classification_loss(logits1, prior_scores, labels)
            cls_loss2, n_p = self.compute_classification_loss(logits2, prior_scores, labels)
            loss_dict = dict(cls_loss1=cls_loss1, cls_loss2=cls_loss2)
            memory[0] = self.patch_head(memory[0])
            memory[1] = self.patch_head(memory[1])
            return loss_dict, n_p, memory, logits1[-1], logits2[-1]
...
def build_detector(args, obj_to_verb, net_type:str):
    detr_meta = {}
    ...
    detr_meta["detr"] = detr
    detr_meta["postprocessor"] = postprocessors
    ...
    feature_head = FeatureHead(
        ...
        return_layer, args.triplet_enc_layers, net_type
    )
    model = PViC(
        args.detector,
